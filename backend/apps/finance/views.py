...
from .serializers import (
    BatchCreateSerializer, BudgetCreateSerializer, BudgetListSerializer, BudgetSerializer,
    FinancialAlertSerializer, FinancialEntrySerializer,
    FiscalYearCreateSerializer, FiscalYearListSerializer, FiscalYearSerializer,
    PaymentBatchSerializer, PaymentCancelSerializer, PaymentCreateSerializer,
    PaymentListSerializer, PaymentMarkPaidSerializer, PaymentSerializer,
)
from .services import (
    BudgetService, FinancialReportService, FinanceValidationError,
    FiscalYearService, PaymentBatchService, PaymentService,
)
...
def finance_error_response(e: FinanceValidationError):
    return Response({"status":"error","code":getattr(e,"code","FINANCE_ERROR"),"message":e.message}, status=status.HTTP_400_BAD_REQUEST)
...
class FiscalYearViewSet(ModelViewSet):
    ...
    def get_serializer_class(self):
        if self.action == "list":   return FiscalYearListSerializer
        if self.action == "create": return FiscalYearCreateSerializer
    ...
    def create(self, request, *args, **kwargs):
        ...
        return Response({"status":"success","data":FiscalYearSerializer(fy).data}, status=status.HTTP_201_CREATED)

    @action(detail=False, methods=["get"], url_path="active")
    def active(self, request):
        fy = self.svc.get_active_year()
        if not fy:
            return Response({"status":"success","data":None,"message":"Aucun exercice fiscal actif."})
...
class BudgetViewSet(ModelViewSet):
    ...
    def get_serializer_class(self):
        ...
        if self.action == "create": return BudgetCreateSerializer
    ...
    def get_queryset(self):
        ...
        if fy := p.get("fiscal_year"):   qs = qs.filter(fiscal_year_id=fy)
        if bt := p.get("benefit_type"):  qs = qs.filter(benefit_type_id=bt)
        if dept:= p.get("department"):   qs = qs.filter(department_id=dept)
    ...
    def create(self, request, *args, **kwargs):
        ...
        try:
            ...
            return finance_error_response(e)
        return Response({"status":"success","data":BudgetSerializer(b).data}, status=status.HTTP_201_CREATED)
...
class PaymentViewSet(ModelViewSet):
    ...
    def get_serializer_class(self):
        if self.action == "list":      return PaymentListSerializer
        if self.action == "create":    return PaymentCreateSerializer
        if self.action == "mark_paid": return PaymentMarkPaidSerializer
        if self.action == "cancel":    return PaymentCancelSerializer
    ...
    def create(self, request, *args, **kwargs):
        ...
        try:
            ...
            pay = self.svc.create_from_benefit(benefit, budget, fy, user=request.user, request=request)
        except FinanceValidationError as e:
            return finance_error_response(e)
        except Exception as e:
            return Response({"status":"error","message":str(e)}, status=400)
        return Response({"status":"success","data":PaymentSerializer(pay).data}, status=status.HTTP_201_CREATED)
    ...
    def export(self, request):
        ...
        filters = dict(
            ...
            fiscal_year_id=p.get("fiscal_year",""), budget_id=p.get("budget",""),
            date_from=p.get("date_from",""), date_to=p.get("date_to",""),
        )
        AuditService().log_export(user=request.user, content_type="payments",
                                  filters=filters, request=request)
        rows = FinancialReportService().get_payment_export_data({k:v for k,v in filters.items() if v})
...
class PaymentBatchViewSet(ModelViewSet):
    queryset         = PaymentBatch.objects.filter(is_deleted=False).select_related("fiscal_year","approved_by")
    ...
    def create(self, request, *args, **kwargs):
        ...
        try:
            ...
            batch = self.svc.create(
                data={"label":d["label"],"fiscal_year":fy,"payment_method":d["payment_method"],
            ...
            return finance_error_response(e)
        return Response({"status":"success","data":PaymentBatchSerializer(batch).data}, status=status.HTTP_201_CREATED)
...
class FinancialEntryViewSet(ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        qs = FinancialEntry.objects.select_related("fiscal_year","payment","budget","created_by_user")
        p  = self.request.query_params
        if fy := p.get("fiscal_year"): qs = qs.filter(fiscal_year_id=fy)
        if et := p.get("entry_type"):  qs = qs.filter(entry_type=et)
        if pay:= p.get("payment"):     qs = qs.filter(payment_id=pay)
...
class FinancialAlertViewSet(ModelViewSet):
    ...
    def get_queryset(self):
        ...
        p  = self.request.query_params
        if p.get("unresolved"): qs = qs.filter(is_resolved=False)
        if sv := p.get("severity"): qs = qs.filter(severity=sv)
        if fy := p.get("fiscal_year"): qs = qs.filter(fiscal_year_id=fy)
