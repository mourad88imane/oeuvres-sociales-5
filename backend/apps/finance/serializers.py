...
class FiscalYearListSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model  = FiscalYear
        fields = ["id","year","label","start_date","end_date","status","status_display",
                  "total_budget","total_paid","total_committed","consumption_rate",
                  "available_budget","is_active"]
...
class FiscalYearSerializer(BaseModelSerializer):
    ...
    class Meta:
        ...
        fields = ["id","year","label","start_date","end_date","status","status_display",
                  "total_budget","total_paid","total_committed","consumption_rate",
                  "available_budget","is_active","notes","closed_at","created_at","updated_at"]
        read_only_fields = ["id","total_paid","total_committed","closed_at","created_at","updated_at"]
...
class BudgetListSerializer(serializers.ModelSerializer):
    ...
    benefit_type_name = serializers.CharField(source="benefit_type.name", read_only=True, default="")
    department_name   = serializers.CharField(source="department.name",   read_only=True, default="")
    fiscal_year_label = serializers.CharField(source="fiscal_year.label", read_only=True)
    status_display    = serializers.CharField(source="get_status_display", read_only=True)
    class Meta:
        model  = Budget
        fields = ["id","code","label","fiscal_year","fiscal_year_label","benefit_type","benefit_type_name",
                  "department","department_name","status","status_display","allocated_amount",
                  "paid_amount","committed_amount","reserved_amount","available_amount",
                  "consumption_rate","is_alert_triggered","is_overrun","alert_threshold_pct","anomaly_score"]
...
class BudgetSerializer(BudgetListSerializer):
    ...
    class Meta(BudgetListSerializer.Meta):
        fields = BudgetListSerializer.Meta.fields + [
            ...
            "variance_percent","prediction_data","created_at","updated_at","created_by_name",
        ]
...
class BudgetCreateSerializer(serializers.ModelSerializer):
    class Meta:
        model  = Budget
        fields = ["code","label","fiscal_year","benefit_type","department",
...
class PaymentListSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model  = Payment
        fields = ["id","reference","employee","employee_name","employee_matricule","department_name",
                  "benefit","benefit_ref","benefit_type_name","budget","budget_code",
                  "amount","fees","net_amount","status","status_display",
                  "payment_method","method_display","bank_reference",
                  "scheduled_date","executed_date","anomaly_flag","anomaly_score","created_at"]
...
class PaymentSerializer(PaymentListSerializer):
    ...
    class Meta(PaymentListSerializer.Meta):
        fields = PaymentListSerializer.Meta.fields + [
            "fiscal_year","fiscal_year_label","bank_name","bank_account","value_date",
            "approved_by","approved_by_name","approved_at","accounting_entry","cost_center",
            "notes","risk_indicators","updated_at","created_by_name",
        ]
    def get_approved_by_name(self, obj):
        return obj.approved_by.get_full_name() if obj.approved_by else None
    def get_created_by_name(self, obj):
...
class PaymentBatchSerializer(BaseModelSerializer):
    ...
    class Meta:
        model  = PaymentBatch
        fields = ["id","reference","label","fiscal_year","fiscal_year_label","status","status_display",
                  "payment_method","method_display","total_amount","payment_count",
                  "scheduled_date","executed_date","bank_reference",
                  "approved_by","approved_by_name","approved_at","notes","created_at","updated_at"]
        read_only_fields = ["id","reference","total_amount","payment_count","created_at","updated_at"]
    def get_approved_by_name(self, obj):
        return obj.approved_by.get_full_name() if obj.approved_by else None
...
class FinancialEntrySerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ["id","entry_number","entry_type","entry_type_display","label","amount",
                  "entry_date","accounting_date","debit_account","credit_account",
                  "payment","budget","created_by_name","notes","created_at"]
    def get_created_by_name(self, obj):
...
class FinancialAlertSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ["id","alert_type","alert_type_display","severity","severity_display",
                  "title","message","is_read","is_resolved","ai_generated","ai_confidence",
                  "fiscal_year","budget","payment","created_at"]
