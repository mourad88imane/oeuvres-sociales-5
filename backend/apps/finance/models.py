...
class FiscalYear(BaseModel):
    ...
    total_budget = models.DecimalField(
        max_digits=18, decimal_places=2, default=Decimal("0"), verbose_name="Budget total (DZD)"
    )
    total_paid = models.DecimalField(
        max_digits=18, decimal_places=2, default=Decimal("0"), verbose_name="Total payé (DZD)"
    )
    total_committed = models.DecimalField(
        max_digits=18, decimal_places=2, default=Decimal("0"), verbose_name="Total engagé (DZD)"
    )
    ...
    prediction_data = models.JSONField(
        default=dict, blank=True, help_text="Prévisions budgétaires AI pour l'année suivante"
    )
...
class Budget(BaseModel):
    ...
    fiscal_year = models.ForeignKey(
        FiscalYear, on_delete=models.PROTECT, related_name="budgets", verbose_name="Exercice fiscal"
    )
    ...
    allocated_amount = models.DecimalField(
        max_digits=15, decimal_places=2, verbose_name="Montant alloué (DZD)"
    )
    committed_amount = models.DecimalField(
        max_digits=15, decimal_places=2, default=Decimal("0"), verbose_name="Montant engagé (DZD)"
    )
    paid_amount = models.DecimalField(
        max_digits=15, decimal_places=2, default=Decimal("0"), verbose_name="Montant payé (DZD)"
    )
    reserved_amount = models.DecimalField(
        max_digits=15, decimal_places=2, default=Decimal("0"), verbose_name="Montant réservé (DZD)"
    )
    ...
    variance_percent = models.FloatField(
        null=True, blank=True, help_text="Écart budget vs réel (%) — feature ML"
    )
    anomaly_score = models.FloatField(
        null=True, blank=True, help_text="Score d'anomalie IA (0-100)"
    )
    prediction_data = models.JSONField(
        default=dict, blank=True, help_text="Prévisions et recommandations IA"
    )
    ...
    def available_amount(self) -> Decimal:
        return (
            self.allocated_amount - self.paid_amount - self.committed_amount - self.reserved_amount
        )
    ...
    def consumption_rate(self) -> Decimal:
        if not self.allocated_amount or self.allocated_amount == 0:
            return Decimal("0")
        return ((self.paid_amount + self.committed_amount) / self.allocated_amount * 100).quantize(
            Decimal("0.01")
        )
    ...
    def update_totals(self):
        ...
        self.committed_amount = qs.filter(status__in=["pending", "approved"]).aggregate(
            s=Sum("amount")
        )["s"] or Decimal("0")
...
class Payment(BaseModel):
    ...
    fiscal_year = models.ForeignKey(
        FiscalYear, on_delete=models.PROTECT, related_name="payments", verbose_name="Exercice"
    )
    ...
    fees = models.DecimalField(
        max_digits=10, decimal_places=2, default=Decimal("0"), verbose_name="Frais bancaires (DZD)"
    )
    net_amount = models.DecimalField(
        max_digits=14, decimal_places=2, default=Decimal("0"), verbose_name="Montant net (DZD)"
    )
    ...
    # Comptabilité
    accounting_entry = models.CharField(
        max_length=50, blank=True, verbose_name="N° écriture comptable"
    )
    ...
    risk_indicators = models.JSONField(
        default=dict, blank=True, help_text="Indicateurs de risque IA"
    )
...
class FinancialEntry(BaseModel):
    ...
    payment = models.ForeignKey(
        Payment, on_delete=models.PROTECT, null=True, blank=True, related_name="accounting_entries"
    )
    budget = models.ForeignKey(
        Budget, on_delete=models.PROTECT, null=True, blank=True, related_name="accounting_entries"
    )

    entry_number = models.CharField(
        max_length=30, unique=True, editable=False, verbose_name="N° écriture"
    )
    ...
    def save(self, *args, **kwargs):
        ...
        if not self._state.adding:
            raise PermissionError(
                "Les écritures comptables sont immuables. Créez une écriture d'annulation."
            )
        super().save(*args, **kwargs)
...
class FinancialAlert(BaseModel):
    ...
    ai_confidence = models.FloatField(
        null=True, blank=True, help_text="Confiance du modèle AI (0-1)"
    )
