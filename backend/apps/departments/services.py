...
class DepartmentService:
    ...
    @transaction.atomic
    def update(self, dept: Department, validated_data: dict, user=None, request=None) -> Department:
        ...
        if user:
            dept.updated_by = user
        dept.save()
        audit.log_update(user=user, obj=dept, before_data=before, request=request)
        return dept

    @transaction.atomic
    def delete(self, dept: Department, user=None, request=None):
        # Vérifier qu'il n'y a pas d'employés actifs
        active_count = dept.employees.filter(status="active", is_deleted=False).count()
        if active_count:
