...
from .models import Benefit, BenefitAttachment, BenefitComment, BenefitType
...
class BenefitService:
    ...
    def get_queryset(self):
        return (
            ...
            .select_related(
                ...
                "benefit_type", "beneficiary",
                "validated_by", "paid_by",
                "created_by", "last_transition_by",
            )
    ...
    def search(
        ...
        if is_overdue:
            ...
            ).exclude(workflow_state__in=["paid","rejected","cancelled"])

        allowed_orderings = {
            "-created_at","created_at","-submitted_at","submitted_at",
            "-requested_amount","requested_amount","priority",
            "-workflow_state","workflow_state","reference","-reference",
        }
    ...
    def transition(
        ...
        benefit.save(update_fields=["last_transition_at","last_transition_by","last_transition_reason"])

        audit.log_workflow_transition(
            user=user, obj=benefit,
    ...
    # ── Pièces jointes ────────────────────────────────────
    @transaction.atomic
    def add_attachment(self, benefit: Benefit, file, doc_type: str, description: str, user=None) -> BenefitAttachment:
        att = BenefitAttachment(
            benefit=benefit, file=file,
            original_name=file.name, doc_type=doc_type,
        ...
        att.save()
        audit.log_action(
            action="CREATE", user=user, obj=att,
    ...
    def add_comment(
        ...
        content: str, comment_type: str = "internal",
        user=None,
    ) -> BenefitComment:
        comment = BenefitComment(
            benefit=benefit, author=user,
    ...
    def get_statistics(self, filters: dict = None) -> dict:
        ...
        stats = {
            ...
            "total_approved":  float(qs.filter(approved_amount__isnull=False).aggregate(s=Sum("approved_amount"))["s"] or 0),
    ...
    def _avg_processing_days(self, qs) -> float | None:
        result = qs.filter(submitted_at__isnull=False, paid_at__isnull=False).extra(
            select={"days": "EXTRACT(EPOCH FROM (paid_at - submitted_at)) / 86400"}
        ).aggregate(avg=Avg("days"))
        return round(result["avg"], 1) if result.get("avg") else None
    ...
    def _compute_sla_rate(self, paid_qs) -> float | None:
        ...
        total = paid_qs.filter(
            benefit_type__target_processing_days__isnull=False
        ).count()
        if not total:
            return None
        on_time = paid_qs.filter(
            benefit_type__target_processing_days__isnull=False,
        ).extra(
            where=["EXTRACT(EPOCH FROM (paid_at - submitted_at)) / 86400 <= apps_benefits_benefittype.target_processing_days"]
        ).count()
    ...
    def get_employee_benefits_summary(self, employee_id: str) -> dict:
        ...
        return {
            "total": qs.count(),
            "pending": qs.filter(workflow_state__in=["draft","submitted","under_review","on_hold"]).count(),
            "paid": qs.filter(workflow_state="paid").count(),
            "total_paid_amount": float(qs.filter(workflow_state="paid").aggregate(s=Sum("paid_amount"))["s"] or 0),
            "by_type": list(qs.values("benefit_type__name","benefit_type__category").annotate(count=Count("id"))),
        }
