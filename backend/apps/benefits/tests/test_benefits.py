...
def admin_user(db):
    return User.objects.create_user(
        email="admin@t.dz", password="P@ss123!", first_name="A", last_name="D", role="admin"
    )
...
def gestionnaire(db):
    return User.objects.create_user(
        email="gest@t.dz", password="P@ss123!", first_name="G", last_name="D", role="gestionnaire"
    )
...
def comptable(db):
    return User.objects.create_user(
        email="comp@t.dz", password="P@ss123!", first_name="C", last_name="D", role="comptable"
    )
...
class TestWorkflowEngine:
    ...
    def test_full_workflow_cycle(self, benefit, admin_user, gestionnaire, comptable):
        ...
        svc.transition(benefit, "validated", user=gestionnaire, metadata={"approved_amount": 45000})
        svc.transition(
            benefit,
            "paid",
            user=comptable,
...
class TestBenefitAPI:
    ...
    def test_transition_via_api(self, api, admin_user, benefit):
        client = auth(api, admin_user)
        resp = client.post(
            f"/api/v1/benefits/{benefit.id}/transition/", {"to_state": "submitted"}, format="json"
        )
    ...
    def test_invalid_transition_returns_400(self, api, admin_user, benefit):
        client = auth(api, admin_user)
        resp = client.post(
            f"/api/v1/benefits/{benefit.id}/transition/", {"to_state": "paid"}, format="json"
        )
    ...
    def test_full_api_cycle(self, api, admin_user, gestionnaire, comptable, employee, benefit_type):
        ...
        # Soumettre
        r = g_client.post(
            f"/api/v1/benefits/{bid}/transition/", {"to_state": "submitted"}, format="json"
        )
        assert r.status_code == 200

        # Prendre en charge
        r = g_client.post(
            f"/api/v1/benefits/{bid}/transition/", {"to_state": "under_review"}, format="json"
        )
        assert r.status_code == 200

        # Valider
        r = g_client.post(
            f"/api/v1/benefits/{bid}/transition/",
            {"to_state": "validated", "approved_amount": 50000},
            format="json",
        )
        assert r.status_code == 200
        ...
        r = c_client.post(
            f"/api/v1/benefits/{bid}/transition/",
            {"to_state": "paid", "payment_reference": "VIR-TEST-001", "paid_amount": 50000},
            format="json",
        )
        assert r.status_code == 200
