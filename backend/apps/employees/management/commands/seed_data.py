...
FIRST_NAMES_M = ["Ahmed","Mohammed","Karim","Youcef","Hocine","Sofiane","Farouk","Rachid","Omar","Salim"]
FIRST_NAMES_F = ["Fatima","Amina","Leila","Nassima","Meriem","Samira","Karima","Nadia","Sara","Wafa"]
LAST_NAMES    = ["Benali","Kaci","Amari","Meziane","Taleb","Ferhat","Ouali","Cherif","Hadj","Bensalem"]

JOBS = [
    ("Ingénieur principal",  "A", 8), ("Ingénieur",           "A", 5),
    ("Technicien supérieur", "B", 6), ("Technicien",          "B", 4),
    ("Chef de service",      "A", 9), ("Agent administratif", "C", 3),
    ("Comptable",            "B", 5), ("Analyste financier",  "B", 6),
    ("Responsable RH",       "A", 7), ("Gestionnaire",        "B", 5),
]
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument("--employees", type=int, default=30,
                            help="Nombre d'employés à créer (défaut: 30)")
        parser.add_argument("--clear", action="store_true",
                            help="Supprimer les données existantes avant de seeder")
    ...
    def _create_users(self):
        ...
        if not User.objects.filter(is_superuser=True).exists():
            ...
            self.stdout.write(f"  🔑 Superadmin: superadmin@oeuvres.dz / SuperAdmin123!")
...
def seed_benefits(emp_list, admin_user):
    ...
    for code, name, cat, maxamt, minseni in types:
        t, _ = BenefitType.objects.get_or_create(
            code=code,
            defaults={"name":name,"category":cat,"max_amount":maxamt,"min_seniority_years":minseni,
                      "target_processing_days":5,"is_active":True,"created_by":admin_user}
        )
        type_objs.append(t)
    ...
    for emp in emp_list[:20]:
        ...
        b = Benefit.objects.create(
            employee=emp, benefit_type=btype,
            title=f"{btype.name} — {emp.get_full_name()}",
            requested_amount=amount, priority=random.choice(["low","normal","normal","high"]),
            workflow_state="draft", created_by=admin_user,
        )
        try:
            ...
            if state in ("validated","paid"):
                ...
                svc.transition(b, "validated",    user=admin_user, metadata={"approved_amount":float(amount)})
            if state == "paid":
                svc.transition(b, "paid", user=admin_user, metadata={"payment_reference":f"VIR-TEST-{b.reference}","paid_amount":float(amount)})
            if state == "rejected":
