...
from .serializers import BeneficiarySerializer, BeneficiaryCreateSerializer, BeneficiaryUpdateSerializer
...
class BeneficiaryViewSet(ModelViewSet):
    ...
    def get_serializer_class(self):
        if self.action == "create":                       return BeneficiaryCreateSerializer
        if self.action in ("update","partial_update"):    return BeneficiaryUpdateSerializer
    ...
    def get_queryset(self):
        return Beneficiary.objects.filter(employee=self._get_employee(), is_deleted=False).order_by("relationship","last_name")

    def list(self, request, *args, **kwargs):
        emp = self._get_employee()
        qs  = self.get_queryset()
        return Response({"status":"success","employee":{"id":str(emp.id),"matricule":emp.matricule,"name":emp.get_full_name()},"count":qs.count(),"data":BeneficiarySerializer(qs, many=True, context={"request":request}).data})

    def create(self, request, *args, **kwargs):
        emp = self._get_employee()
        ser = BeneficiaryCreateSerializer(data=request.data, context={"request":request,"employee":emp})
        ser.is_valid(raise_exception=True)
        bene = service.create(emp, ser.validated_data, user=request.user, request=request)
        return Response({"status":"success","data":BeneficiarySerializer(bene, context={"request":request}).data}, status=status.HTTP_201_CREATED)

    def update(self, request, *args, **kwargs):
        partial = kwargs.pop("partial", False)
        bene = self.get_object()
        ser  = BeneficiaryUpdateSerializer(bene, data=request.data, partial=partial, context={"request":request,"employee":bene.employee})
        ser.is_valid(raise_exception=True)
        bene = service.update(bene, ser.validated_data, user=request.user, request=request)
        return Response({"status":"success","data":BeneficiarySerializer(bene, context={"request":request}).data})
...
class BeneficiaryGlobalViewSet(ReadOnlyModelViewSet):
    queryset = Beneficiary.objects.filter(is_deleted=False).select_related(
