...
class BaseReportScheduleSerializer(serializers.ModelSerializer):
    class Meta:
        fields = [
            "id", "frequency", "cron_expression", "recipients",
            "format", "is_active", "last_run", "next_run", "filters_override",
        ]
...
class BaseDataExportSerializer(serializers.ModelSerializer):
    class Meta:
        fields = [
            "id", "export_format", "status", "file", "file_size",
            "filters_used", "row_count", "error_message",
...
class BaseKpiSnapshotSerializer(serializers.ModelSerializer):
    class Meta:
        fields = [
            "id", "kpi", "value", "date", "previous_value",
            "variation", "metadata",
        ]
...
class BaseDashboardWidgetSerializer(serializers.ModelSerializer):
    class Meta:
        fields = [
            "id", "title", "widget_type", "size", "config",
