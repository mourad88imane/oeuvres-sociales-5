...
from .models import AIModelRegistry, AIRecommendation
...
class RecommendationEngine:
    ...
    def _budget_recommendations(self) -> list[dict]:
        ...
        high_var = budgets.annotate(variance=Avg("variance_percent")).filter(variance__gt=20)[:5]
        for b in high_var:
            recs.append(
                {
                    "domain": "finance",
    ...
    def collaborative_recommendations(self, user, limit: int = 5) -> list[dict]:
        ...
        recent_events = (
            AIEvent.objects.filter(user=user).values("target_type", "event_name").distinct()
        )
        similar_users = (
                ...
                recs.append(
                    {
                        ...
                        "detail": "Recommandé d'après l'activité d'utilisateurs similaires.",
    ...
    def _get_model(self):
        model, _ = AIModelRegistry.objects.get_or_create(
            ...
            defaults={
                ...
                "algorithm": "rule-based + collaborative",
                "status": "production",
            },
        )
        return model
