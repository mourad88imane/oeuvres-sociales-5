...
from .models import AIAnomaly, AIEvent
...
class BehaviorAnalyzer:
    ...
    def analyze_user_activity(self, user, days: int = 30) -> dict:
        ...
        by_category = list(
            events.values("event_category").annotate(count=Count("id")).order_by("-count")
        )
        by_hour = list(
    ...
    def segment_users(self, days: int = 30) -> list[dict]:
        ...
        return [
            {
                "segment": "Power users",
                "count": len(segments["power"]),
                "description": "> 100 événements/30j",
            },
            {
                "segment": "Utilisateurs réguliers",
                "count": len(segments["regular"]),
                "description": "30-100 événements/30j",
            },
            {
                "segment": "Utilisateurs occasionnels",
                "count": len(segments["occasional"]),
                "description": "5-30 événements/30j",
            },
            {
                "segment": "Utilisateurs inactifs",
                "count": len(segments["inactive"]),
                "description": "< 5 événements/30j",
            },
        ]
    ...
    def user_risk_profile(self, user) -> dict:
        ...
        events_30d = AIEvent.objects.filter(
            user=user, timestamp__gte=timezone.now() - timedelta(days=30)
        ).count()
        anomalies = AIAnomaly.objects.filter(target_type="User", target_id=str(user.pk)).count()
        failed_logins = AIEvent.objects.filter(
            user=user, event_name="login_failed", timestamp__gte=timezone.now() - timedelta(days=7)
        ).count()
        ...
        if events_30d > 200:
            factors.append(
                {
                    "name": "activité_élevée",
                    "impact": "+20",
                    "detail": f"{events_30d} événements en 30j",
                }
            )
        if anomalies > 0:
            factors.append(
                {
                    "name": "anomalies_passées",
                    "impact": f"+{anomalies * 15}",
        ...
        if failed_logins > 3:
            factors.append(
                {
                    "name": "échecs_connexion",
                    "impact": f"+{failed_logins * 10}",
                    "detail": f"{failed_logins} échecs en 7j",
                }
            )
